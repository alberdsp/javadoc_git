...
/**
 * Clase TratamientoFichero para tratar los fichero tanto de clientes como de
 * visitas ruta_clientes establece la ruta donde se guradan los clientes
...
// Añadir flag a true para no machacar contenido del fichero de escritura
			...
			for (Entry<String, Visita> listav : listavisitas.entrySet()) {
				...
				pw.println(dni + delimitador + dniProfesional + delimitador + fecha + delimitador + hora + delimitador
						+ peso + delimitador + altura + delimitador + unidadaltura + delimitador + resulimc);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void grabarProfesionales(TreeMap<String, Profesionales_Medicos> profesionales) {
		...
		try {
			...
			for (Entry<String, Profesionales_Medicos> listaP : listaProfesionales.entrySet()) {
				...
				pw.println(dni + delimitador + nombre + delimitador + apellidos + delimitador + localidad + delimitador
						+ telefono + delimitador + especialidad);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	/**
	 * Método buscar un Profesional, busca en el fichero de Profesionales hasta que
	 * encuentra el DNI pasado como parametro String ndni en el fichero indicado o
	 * bien llega al final del fichero y nos devuelve el objeto paciente vacio
	 * 
	 * @param ndni es el número de dni
